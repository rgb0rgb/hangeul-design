...
def test_python_sources_parse():
    for name in ["app.py", "app_cinematic.py", "cinematic_product.py", "reference_image.py", "runtime_quality.py", "launcher.py", "prompt_compiler.py", "beginner_mode.py", "result_assistant.py"]:
...
def test_expert_avoid_clause_is_extracted():
    ...
    assert "extra fingers" not in positive and "duplicated faces" not in positive
    assert "extra fingers" in negative and "duplicated faces" in negative
...
def test_hangul_typography_does_not_apply_blanket_text_ban():
    ...
    low = out.lower()
    assert "--no text" not in low and "--no letters" not in low
    assert "broken-typography" in low and '"한글 디자인"' in out
...
def test_video_compiler_applies_aspect_and_model_guidance():
    ...
    assert "Frame aspect ratio: 9:16" in out and "Target Kling" in out
...
def test_symptom_correction_is_idempotent():
    first = prompt_compiler.apply_prompt_correction("premium watch --ar 4:5 --no watermark", "제품이 달라짐")
    second = prompt_compiler.apply_prompt_correction(first, "제품이 달라짐")
    assert second == first
...
def test_variant_respects_explicit_camera_and_lighting():
    ...
    assert "lighting variation:" not in text and "camera variation:" not in text and "palette variation:" not in text
...
def test_install_is_idempotent_and_originals_are_captured_once():
    ...
    runtime_quality.install(app)
    assert app._build_prompts is first_quick and app._build_expert_prompt is first_expert
    assert app._hd_original_build_prompts is original_quick and app._hd_original_build_expert_prompt is original_expert
...
def test_repeated_generation_has_exactly_one_variant_prefix():
    ...
    assert first.count("Variant ") == 1 and second.count("Variant ") == 1
    assert first != second
...
def test_license_is_noncommercial_personal_use():
    ...
    assert "personal, non-commercial" in text and "Commercial Use Prohibited" in text
