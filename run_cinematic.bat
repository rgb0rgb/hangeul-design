...
start "Hangeul Design Cinematic Server" /MIN "%CD%\venv\Scripts\python.exe" -m streamlit run "%CD%\app_cinematic.py" --server.address localhost --server.port %PORT% --server.headless false
if errorlevel 1 goto fail

echo Cinematic starting on http://localhost:%PORT%
endlocal
exit /b 0
