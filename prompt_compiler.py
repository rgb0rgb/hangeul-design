...
def _split_exclusions(prompt: str) -> Tuple[str, list[str]]:
    """Extract the final avoid/exclude clause from quick or expert prompts.

    The compiler runs before reference-image text is appended, but this parser also
    tolerates a later REFERENCE IMAGE CONTROL block so reference use cannot disable
    negative extraction.
    """
...
def _clean_text_negatives(items: list[str], allow_text: bool) -> list[str]:
    if not allow_text:
        return items
    # Remove only blanket text bans. Keep useful quality constraints such as
    # garbled text, misspelled Hangul, broken typography, or random characters.
    ...
    if allow_text:
        ...
        if exact:
            body += (
                ...
                "Keep Hangul glyph structure, spacing, and readability accurate. "
...
def apply_prompt_correction(prompt: str, symptom: str) -> str:
    """Apply a symptom-based correction while preserving terminal model parameters."""
    corrections = {
